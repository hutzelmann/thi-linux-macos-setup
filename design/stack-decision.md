...
- Weaknesses: sidebar must be hand-written or generated via `vitepress-sidebar`
  (community); small plugin ecosystem; **no exact-match search** — MiniSearch is fuzzy by
  design, long-standing open request. Tune `fuzzy: 0.1` if flag/package names get noisy.
...
- `<Tabs groupId="os">` syncs across the whole page and persists in localStorage —
  solves R10 for free, which VitePress does not.
...
locale** — a German query must not surface English hits.
