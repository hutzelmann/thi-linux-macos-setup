...
| R7 | Scripts and containers live in the repo as real, testable files and are embedded into docs — no copy-paste duplication | Hard |
...
| R17 | Readers can see how recently a page was last checked, per OS — stated as an observation, never as a guarantee (R29) | Soft |
| R18 | Verification requiring campus-network presence must run from inside that network | Hard |
| R19 | OS/package drift is **out of scope** — do not build for it | — |
...
| R31 | Students and staff are served **equally** — no assumed technical background, neither group second-class | Hard |
...
**R11 — detection collapses to one check: "is this a Mac?"** UA cannot reveal a Linux
...
- **Debian users get a silent wrong default.** A UI problem, not a detection one — the
...
| 5 | End-to-end / physical outcome | — | human |
...
- **One-command reproduction.** `./debug.sh <check-id>` spins the right container, runs the
  check, leaves the agent inside. Repo mounted **read-only** plus a scratch dir — the agent
  proposes diffs instead of mutating source while experimenting.
...
- [ ] **Ask IT for a low-privilege test account** — unlocks layer 4 automation
